"""
APDA MITRA — Region V1 XGBoost Model Trainer (10 Target States)
===============================================================
Trains and rigorously validates the XGBoost Landslide Risk Model across the
10 Himalayan & Northeast states using realistic Temporal Blocking:

- Train Set:      2000–2021 (Historical learning base)
- Validation Set: 2022–2023 (Model tuning & early stopping)
- Test Set:       2024–2025 (Completely unseen future evaluation)
...
Outputs:
- backend/ai/datasets/models/region_v1/apda_mitra_xgboost.pkl
- backend/ai/datasets/models/region_v1/scaler.joblib
- backend/ai/datasets/models/region_v1/features.json
- backend/ai/datasets/models/region_v1/model_metadata.json
"""
...
REGION_V1_MODEL_DIR = MODELS_DIR / "region_v1"
LATEST_MODEL_DIR = MODELS_DIR / "latest"

REGION_V1_MODEL_DIR.mkdir(parents=True, exist_ok=True)
...
def train_and_validate_region_v1():
    print("=" * 75)
    print("APDA MITRA — TRAINING REGION V1 XGBOOST (10 STATES, TEMPORAL SPLIT)")
    print("=" * 75)
    ...
    print(f"[*] Loaded Region V1 dataset with {len(df)} total samples.")
    ...
    print(f"    - Train Set      (2000-2021): {len(X_train)} samples ({y_train.sum()} landslides)")
    print(f"    - Validation Set (2022-2023): {len(X_val)} samples ({y_val.sum()} landslides)")
    print(f"    - Test Set       (2024-2025): {len(X_test)} samples ({y_test.sum()} landslides)")

    # 2. Scaling (Fit strictly on Train only)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_val_scaled = scaler.transform(X_val)
    X_test_scaled = scaler.transform(X_test)

    # Calculate class weight ratio for rare-event calibration
    ...
    print("\n[*] Fitting XGBoost with early stopping on validation PR-AUC...")
    model.fit(
        X_train_scaled,
        y_train,
    ...
    # 4. Evaluation on Completely Unseen Future Test Set (2024–2025)
    ...
    print("\n" + "=" * 75)
    print("UNSEEN FUTURE TEST SET EVALUATION (2024–2025)")
    print("=" * 75)
    ...
    pkl_path = REGION_V1_MODEL_DIR / "apda_mitra_xgboost.pkl"
    with open(pkl_path, "wb") as f:
        pickle.dump(model, f)

    joblib_path = REGION_V1_MODEL_DIR / "apda_mitra_xgboost.joblib"
    joblib.dump(model, joblib_path)

    scaler_path = REGION_V1_MODEL_DIR / "scaler.joblib"
    ...
    features_path = REGION_V1_MODEL_DIR / "features.json"
    with open(features_path, "w", encoding="utf-8") as f:
        json.dump(features_meta, f, indent=2)

    metadata = {
        "model_name": "Apda Mitra XGBoost Regional Landslide Predictor (Region V1)",
        "algorithm": "XGBoost (Extreme Gradient Boosting)",
        "version": "1.0.0",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "geographic_scope": "10 Himalayan & Northeast States",
        "temporal_evaluation": {
            "train_period": "2000-2021",
    ...
    metadata_path = REGION_V1_MODEL_DIR / "model_metadata.json"
    with open(metadata_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)

    # Sync to latest for live prediction serving
    ...
    print(f"[OK] Synced latest model:  {LATEST_MODEL_DIR}")
    print("=" * 75)


if __name__ == "__main__":
    train_and_validate_region_v1()
