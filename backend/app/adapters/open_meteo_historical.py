...
class OpenMeteoHistoricalAdapter:
    ...
    async def get_historical_weather(
        ...
        safe_lookback = max(7, min(35, lookback_days))
    ...
    def _parse_historical_payload(
        ...
        # Calculate metrics and multi-window rainfall accumulations
        all_precips = [s["precipitation_mm"] for s in history_steps]
        total_rainfall = sum(all_precips)
        precip_1d = sum(all_precips[-1:]) if len(all_precips) >= 1 else total_rainfall
        precip_3d = sum(all_precips[-3:]) if len(all_precips) >= 3 else total_rainfall
        precip_7d = sum(all_precips[-7:]) if len(all_precips) >= 7 else total_rainfall
        precip_15d = sum(all_precips[-15:]) if len(all_precips) >= 15 else total_rainfall
        precip_30d = sum(all_precips[-30:]) if len(all_precips) >= 30 else total_rainfall
        ...
        return {
            ...
            "rain_1d": round(precip_1d, 2),
            "rain_3d": round(precip_3d, 2),
            "rain_7d": round(precip_7d, 2),
            "rain_15d": round(precip_15d, 2),
            "rain_30d": round(precip_30d, 2),
            "precip_7d_mm": round(precip_7d, 1),
            "precip_14d_mm": round(precip_15d, 1),
